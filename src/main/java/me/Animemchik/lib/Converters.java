...
import static me.Animemchik.lib.ValueUtils.getFloatNumber;
...
public final class Converters {
    ...
    public static FunctionValue booleanOptToVoid(BooleanToVoidFunction f, final boolean def) {
        return new FunctionValue(args -> {
            ...
            f.apply( (args.length == 1) ? (args[0].asInteger() != 0) : def );
            return NumberValue.ZERO;
        });
    }
    ...
    public static FunctionValue intToLong(IntToLongFunction f) {
        return new FunctionValue(args -> {
            Arguments.check(1, args.length);
            return NumberValue.of(f.apply(args[0].asInteger()));
        });
    }
